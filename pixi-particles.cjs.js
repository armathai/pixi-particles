...
class Particle {
    constructor(additive, texture) {
        this.rotation = 0;
        //
        this.position = new pixi_js.Point(0, 0);
        this.positionOffset = new pixi_js.Point(0, 0);
        this.scale = new pixi_js.Point(0, 0);
        this._sprite = new pixi_js.Sprite(texture);
        this._sprite.anchor.set(0.5);
        this._sprite.blendMode = additive ? pixi_js.BLEND_MODES.ADD : pixi_js.BLEND_MODES.NORMAL;
    }
    get sprite() {
        return this._sprite;
    }
...
class ParticleEmitter {
    ...
    _activateParticle(index) {
        ...
        if ((updateFlags & UpdateFlags.angle) === 0) {
            ...
            particle.angleCos = Math.cos(pixi_js.DEG_TO_RAD * angle);
            particle.angleSin = Math.sin(pixi_js.DEG_TO_RAD * angle);
        }
        ...
        switch (this._spawnShapeValue.shape) {
            ...
            case SpawnShape.ellipse: {
                ...
                if (this._spawnShapeValue.edges) {
                    ...
                    const cosDeg = Math.cos(pixi_js.DEG_TO_RAD * angle);
                    const sinDeg = Math.sin(pixi_js.DEG_TO_RAD * angle);
                    x += cosDeg * radiusX;
    ...
    _updateParticle(particle, delta, deltaMillis) {
        ...
        if ((updateFlags & UpdateFlags.velocity) !== 0) {
            ...
            if ((updateFlags & UpdateFlags.angle) !== 0) {
                ...
                velocityX = velocity * Math.cos(pixi_js.DEG_TO_RAD * angle);
                velocityY = velocity * Math.sin(pixi_js.DEG_TO_RAD * angle);
...
class ParticleEffect extends pixi_js.Container {
